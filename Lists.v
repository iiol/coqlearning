...
Example test_add'1:                count 1 (add' 1 [1;4;1]) = 3.
Proof. reflexivity. Qed.
Example test_add'2:                count 5 (add' 1 [1;4;1]) = 0.
Proof. reflexivity. Qed.
...
Definition member (v : nat) (s : bag) : bool := O <? count v s.
...
Proof.
  intros n s. simpl. rewrite eqb_refl. reflexivity.
Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite app_length'', IHl, <- PeanoNat.Nat.add_1_r. reflexivity.
Qed.
...
Proof.
  intros l. induction l as [| h t IHl].
  - reflexivity.
  - simpl. rewrite eqb_refl. apply IHl.
Qed.
(** [] *)
...
Theorem bag_count_sum :
  forall n l1 l2, count n l1 + count n l2 = count n (sum l1 l2).
Proof.
  intros n l1 l2. induction l1 as [| m t IHl].
  - reflexivity.
  - simpl. destruct (m =? n).
    + simpl. rewrite IHl. reflexivity.
    + simpl. rewrite IHl. reflexivity.
...
Proof.
  intros l1 l2 H.
  rewrite <- rev_involutive, <- H. rewrite rev_involutive. reflexivity.
Qed.
