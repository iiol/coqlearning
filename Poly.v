...
Example test_split:
  split [(1,false);(2,true)] = ([1;2],[false;true]).
Proof. reflexivity. Qed.
(** [] *)
...
Definition negf {X : Type} (f : X -> bool) (x : X) : bool :=
  if f x then false else true.

Definition partition {X : Type}
                     (test : X -> bool)
                     (l : list X)
                   : list X * list X := (filter test l, filter (negf test) l).
...
Lemma map_app :
  forall X Y (f : X->Y) l1 l2, map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof.
  intros X Y f l1 l2. induction l1 as [| h t IHl].
  - reflexivity.
  - simpl. rewrite IHl. reflexivity.
Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite map_app, IHl. reflexivity.
Qed.
(** [] *)
...
(* HUI ZNAET where it would be useful but there is some stupid example *)
Definition check {X : Type} (f : X -> X) (b1 : bool) (x : X) : X :=
  if b1 then f x else x.

Compute fold (check rev) [true; false; true] [1; 2; 3; 4].
...
Theorem fold_map_correct :
  forall X Y (f : X->Y) l, map f l = fold_map f l.
Proof.
  intros X Y f l. induction l.
  - reflexivity.
  - simpl. rewrite IHl. reflexivity.
...
Theorem nth_error_length' :
  forall X (l : list X), nth_error l (length l) = None.
Proof.
  intros X l. induction l.
  - reflexivity.
  - simpl. apply IHl.
...
(*
Theorem nth_error_length :
  forall X (l : list X) n, length l = n -> nth_error l n = None.
Proof.
  intros X l n. induction l.
  - reflexivity.
  - simpl. intros H. rewrite <- IHl, <- H. simpl.
 *)
(* FILL IN HERE *)
